            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {liste.map((d) => {
                  ...
                  return (
                    <Fragment key={d.id}>
                      <tr className={`border border-black/10 ${urgence ? "bg-isoRed/5" : ""}`}>
                        <td className="px-3 py-2 border border-black/10 font-medium">
                          {urgence && <span title={`Plus de ${seuilUrgence}h sans vérification`}>🔥 </span>}
                          {d.nom_dossier}
                          {d.recurrent && <span className="badge bg-isoRed/10 text-isoRed ml-2">⚠</span>}
                        </td>
                        ...
                        <td className="px-3 py-2 border border-black/10">{d.client}</td>
                        <td className="px-3 py-2 border border-black/10">
                          <StatutBadge etat={d.etat} />
                          {d.pris_en_charge_par && (
                            <p className="text-[11px] text-ink/40 mt-0.5">par {d.pris_en_charge_par}</p>
                          )}
                        </td>
                        <td className="px-3 py-2 border border-black/10">
                      ...
                      {dossierActif === d.id && (
                        <tr className="border border-black/10">
                          <td colSpan={7} className="px-3 border border-black/10 bg-black/[0.015]">
                            <PanneauAction d={d} />
                          </td>
                        </tr>
                      )}
...
function StatutBadge({ etat }) {
  const map = {
    ...
    "Audité": { texte: "Audité", classe: "bg-isoGreen/10 text-isoGreen" },
