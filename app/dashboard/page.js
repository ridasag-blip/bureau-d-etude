...
import CreerDossierPopup from "@/components/CreerDossierPopup";
...
function DashboardContent() {
  ...
  // Arrivée depuis le lien "Saisie" du menu → ouvre directement la popup
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* Bande logo + KPI, façon CRM Prime */}
        <div className="card p-6 mb-6 flex flex-col md:flex-row gap-6">
          <div className="flex flex-col items-center justify-center gap-2 md:w-48 shrink-0 md:border-r border-black/5 md:pr-6">
            <img src="/logo-hillsolution.png" alt="Hill Solution" className="h-16 w-auto" />
          </div>
        ...
        {/* Ligne de résumé + actions */}
        <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
          <div>
            <p className="font-display font-semibold text-lg">
              Dossiers · {filtres_.length} affichés
            </p>
            <p className="text-sm text-ink/50">Total : {dossiers.length}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={seDeconnecter} className="btn-secondary text-sm">
              Déconnexion
            </button>
            <button onClick={() => setCreerOuvert(true)} className="btn-primary text-sm">
              Créer un dossier
            </button>
          </div>
        </div>
        ...
        <NavigationOnglets
          ...
          onChange={(op) => setFiltres((f) => ({ ...f, operation: op === "Tous" ? "Tous" : op }))}
        />
      ...
      {creerOuvert && (
        <CreerDossierPopup
          ...
          chargeParIngenieur={chargeParIngenieur(dossiers)}
...
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="p-10 text-center text-ink/40">Chargement…</div>}>
      <DashboardContent />
    </Suspense>
  );
}
