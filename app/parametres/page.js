...
export default function ParametresPage() {
  ...
  async function majPin(id, pin) {
    await supabase.from("parametres_ingenieurs").update({ pin }).eq("id", id);
    chargerListe();
  }
                ...
                {lignes.map((l) => (
                  <li key={l.id} className="py-2 flex justify-between items-center text-sm gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      {ongletActif === "parametres_ingenieurs" && (
                        <input
                          type="text"
                          maxLength={8}
                          placeholder="Code PIN"
                          defaultValue={l.pin || ""}
                          onBlur={(e) => majPin(l.id, e.target.value)}
        ...
        {vue === "config" && (
          <div className="card p-4 max-w-sm">
            ...
            <p className="text-sm text-ink/60 mb-3">
              ...
              (badge rouge sur « Vérification » et sur le Dashboard).
            </p>
            <div className="flex items-center gap-2">
              <input
                type="number"
