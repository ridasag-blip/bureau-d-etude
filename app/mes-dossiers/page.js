...
function CompteARebours({ dateAcceptation, delaiMaxHeures }) {
  ...
  return (
    <p className={`text-[11px] mt-0.5 ${depasse ? "text-isoRed font-medium" : "text-isoGold"}`}>
      {depasse
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="text-left text-xs text-ink/50 uppercase bg-black/[0.03]">
                ...
                <th className="px-3 py-2 border border-black/10">Nature</th>
                <th className="px-3 py-2 border border-black/10">Statut</th>
                {avecAnciennete && <th className="px-3 py-2 border border-black/10">Ancienneté</th>}
            ...
            <tbody>
              {liste.map((d) => {
                ...
                return (
                  <tr key={d.id} className={`border border-black/10 ${enRetour ? "bg-isoRed/5" : ""}`}>
                    ...
                    <td className="px-3 py-2 border border-black/10 text-ink/60">{d.nature_prod || "—"}</td>
            ...
            <div className="flex gap-1">
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-medium ${
                  vue === "liste" ? "bg-isoGreen text-white" : "bg-white border"
                }`}
              >
                Liste
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-medium ${
                  vue === "calendrier" ? "bg-isoGreen text-white" : "bg-white border"
                }`}
              >
                Calendrier
              </button>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="text-left text-xs text-ink/50 uppercase bg-black/[0.03]">
                ...
                <th className="px-3 py-2 border border-black/10">Opération</th>
                <th className="px-3 py-2 border border-black/10">Nature</th>
                <th className="px-3 py-2 border border-black/10">État final</th>
                <th className="px-3 py-2 border border-black/10">Validé par</th>
                <th className="px-3 py-2 border border-black/10">Vérifié le</th>
              </tr>
            </thead>
            <tbody>
              {traites.map((d) => (
                <tr key={d.id} className="border border-black/10">
                  <td className="px-3 py-2 border border-black/10 whitespace-nowrap">{d.date}</td>
                  <td className="px-3 py-2 border border-black/10 font-medium">{d.nom_dossier}</td>
                  <td className="px-3 py-2 border border-black/10">{d.nom_operation}</td>
                  <td className="px-3 py-2 border border-black/10 text-ink/60">{d.nature_prod || "—"}</td>
                  <td className="px-3 py-2 border border-black/10">{d.etat}</td>
                  <td className="px-3 py-2 border border-black/10">{d.valide_par || "—"}</td>
                  <td className="px-3 py-2 border border-black/10">
              ...
              {traites.length === 0 && (
                <tr><td colSpan={7} className="px-3 py-8 text-center text-ink/40 border border-black/10">Rien encore.</td></tr>
              )}
            </tbody>
          </table>
        </div>
