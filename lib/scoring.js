...
/**
 * Temps moyen (en heures) entre l'assignation d'un dossier et sa première
 * vérification qualité — objectif cible ~1h.
 */
export function delaiMoyenVerification(dossiers) {
  const delais = dossiers
    .filter((d) => d.date_assignation && d.date_verification)
    .map((d) => (new Date(d.date_verification) - new Date(d.date_assignation)) / 3600000);
...
/** Charge actuelle par ingénieur : nombre de dossiers en Encours non encore vérifiés */
