 ...
 * Excel: MAX(0, 50 * (1 - retours / dossiers_traités))
 */
export function scoreQualite(dossiersTraites, retours) {
  if (!dossiersTraites || dossiersTraites === 0) return null; // "" dans Excel
  return Math.max(0, 50 * (1 - retours / dossiersTraites));
}
...
export function calculerStatsIngenieur(dossiersIngenieur, objectif) {
  ...
  const qualite = scoreQualite(dossierTraiteTotal, nbRetourTotal);
