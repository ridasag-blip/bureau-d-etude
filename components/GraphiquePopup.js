        ...
        {type === "evolution" && (
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={evolutionMensuelle(dossiers)}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="total" stroke="#3CD64A" strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        )}
        ...
        {type === "operation" && (
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={repartitionPar(dossiers, "nom_operation")} layout="vertical" margin={{ left: 20 }}>
              ...
              <Tooltip />
              <Bar dataKey="total" fill="#0E8FE0" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
        ...
        {type === "client" && (
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={repartitionPar(dossiers, "client")} layout="vertical" margin={{ left: 20 }}>
              ...
              <YAxis type="category" dataKey="label" width={110} tick={{ fontSize: 12 }} />
              <Tooltip />
              <Bar dataKey="total" fill="#FFB800" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
        ...
        {type === "nature" && (
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={nouveauxVsModifications(dossiers)}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="Nouveau" fill="#3CD64A" radius={[4, 4, 0, 0]} />
              <Bar dataKey="Modification" fill="#0E8FE0" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
