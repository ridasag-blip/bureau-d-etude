...
export default function AlertesQualite({ dossiers }) {
  ...
  const bloques = dossiers.filter((d) => {
    if (d.etat !== "Encours de vérif") return false;
  ...
  return (
    <div className="card p-4 border-isoRed/30 bg-isoRed/5">
      ...
      <div className="flex flex-col gap-2 text-sm">
        {bloques.length > 0 && (
          <p>
            🕒 <strong>{bloques.length}</strong> dossier(s) en « Encours de vérif » depuis plus de{" "}
            {SEUIL_ALERTE_JOURS_ENCOURS_VERIF} jours.
          </p>
        )}
        {aRisque.length > 0 && (
          <p>
